...
from ..utils.ast_helpers import is_literal, is_string_literal, is_identifier
...
class ControlFlowRecoverer(Transform):
    ...
    def _recover_in_bodies(self, node):
        ...
        bodies_to_check = []
        if ntype == 'Program':
            bodies_to_check.append(('body', node.get('body', [])))
        elif ntype == 'BlockStatement':
            bodies_to_check.append(('body', node.get('body', [])))

        for body_key, body in bodies_to_check:
            self._try_recover_body(node, body_key, body)
        ...
        for key in get_child_keys(node):
            ...
            if isinstance(child, list):
                for item in child:
                    self._recover_in_bodies(item)
            elif isinstance(child, dict) and 'type' in child:
                self._recover_in_bodies(child)
            ...
            if stmt.get('type') == 'VariableDeclaration':
                ...
                if state_info:
                    ...
                    if next_idx < len(body):
                        loop = body[next_idx]
                        recovered = self._try_recover_from_loop(loop, states, state_var, counter_var)
            ...
            if stmt.get('type') == 'ExpressionStatement':
                ...
                if expr and expr.get('type') == 'AssignmentExpression':
                    ...
                    if state_info:
                        ...
                        if next_idx < len(body):
                            loop = body[next_idx]
                            recovered = self._try_recover_from_loop(loop, states, state_var, counter_var or '_index')
                            if recovered is not None:
                                body[i:next_idx + 1] = recovered
                                self.set_changed()
                                continue
    ...
    def _find_state_array_in_decl(self, decl):
        ...
        for d in decl.get('declarations', []):
            ...
            if self._is_split_call(init):
                ...
                state_var = d['id']['name'] if d.get('id', {}).get('type') == 'Identifier' else None
                if states and state_var:
                    ...
                    for d2 in decl.get('declarations', []):
                        ...
                        if d2.get('id', {}).get('type') == 'Identifier':
                            init2 = d2.get('init')
                            if init2 and init2.get('type') == 'Literal' and isinstance(init2.get('value'), (int, float)):
    ...
    def _find_counter_init(self, stmt):
        ...
        if stmt.get('type') == 'ExpressionStatement':
            expr = stmt.get('expression')
            if (expr and expr.get('type') == 'AssignmentExpression' and
                    is_identifier(expr.get('left')) and
                    is_literal(expr.get('right')) and
                    isinstance(expr['right'].get('value'), (int, float))):
    ...
    def _is_split_call(self, node):
        ...
        if not is_string_literal(obj):
            return False
        if not (is_identifier(prop) and prop.get('name') == 'split') and \
           not (is_string_literal(prop) and prop.get('value') == 'split'):
            return False
    ...
    def _try_recover_from_loop(self, loop, states, state_var, counter_var):
        ...
        for case in cases:
            ...
            if test and test.get('type') == 'Literal':
                ...
                stmts = [s for s in case.get('consequent', [])
