...
class ReassignmentRemover(Transform):
    ...
    _WELL_KNOWN_GLOBALS = frozenset({
        ...
        'Promise', 'Symbol', 'Proxy', 'Reflect', 'console', 'parseInt',
        'parseFloat', 'isNaN', 'isFinite', 'Buffer', 'process', 'require',
    ...
    def _process_assignment_aliases(self, scope):
        ...
        for name, binding in list(scope.bindings.items()):
            ...
            reads = []
            for ref_node, ref_parent, ref_key, ref_index in binding.references:
                if (ref_parent and ref_parent.get('type') == 'AssignmentExpression'
                        and ref_key == 'left'):
            ...
            def remove_assign(n, parent, key, index):
                if (n.get('type') == 'ExpressionStatement' and
                        n.get('expression') is write_parent):
                    self.set_changed()
                    return REMOVE
