...
class TestDeobfuscatorExecute:
    ...
    def test_parse_failure_with_hex_escapes_falls_back_to_source_decode(
        self, mock_decode, mock_parse
    ):
    ...
    def test_parse_failure_without_hex_escapes_returns_original(
        self, mock_decode, mock_parse
    ):
    ...
    def test_max_iterations_limits_passes(
        self, mock_generate, mock_parse, mock_transforms
    ):
    ...
    @patch("pyjsclear.deobfuscator.TRANSFORM_CLASSES")
    @patch("pyjsclear.deobfuscator.parse")
    def test_transform_errors_silently_ignored_bug3(
        self, mock_parse, mock_transforms
    ):
        ...
        mock_transforms.__iter__ = lambda self: iter(
            [failing_transform, succeeding_transform]
        )
    ...
    @patch("pyjsclear.deobfuscator.TRANSFORM_CLASSES")
    @patch("pyjsclear.deobfuscator.parse")
    def test_no_transforms_change_anything_returns_original(
        self, mock_parse, mock_transforms
    ):
    ...
    def test_generate_failure_returns_original(
        self, mock_generate, mock_parse, mock_transforms
    ):
